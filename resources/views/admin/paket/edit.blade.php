...
<div class="grid grid-cols-1 gap-4 mb-4">
    <div class="card">
        <div class="header flex items-center">
            <h2 class="header-title">Form Update Data Paket</h2>
           
        </div>
        <div class="body">
            
            <form action="{{ url('admin/paket/edit',$paket->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="grid gap-6 mb-6 md:grid-cols-3">
                    <div>
                        <label for="first_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Paket</label>
                        <input type="text" id="first_name" name="nama" value="{{ $paket->nama }}" class="bg-white/30 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Nama paket ..." required>
                    </div>
                    <div>
                        <label for="first_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Harga Paket</label>
                        <input type="number" id="first_name" name="harga" value="{{ $paket->harga }}" class="bg-white/30 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Harga ..." required>
                    </div>
                    <div>
